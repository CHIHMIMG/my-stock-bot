...
def screen_stocks():
    ...
    print(f"🚀 啟動【原始倍量】掃描... {report_time}")
    ...
    for sid, name in unique_list:
        try:
            ...
            # 判斷條件：今日量 > 6000張 且 今日量 > 昨日量 * 2 且 站上所有均線
            if today_vol >= VOL_THRESHOLD and today_vol >= (yesterday_vol * VOL_RATIO) and close_price >= max(ma5, ma10, ma20, ma60):
                ...
                res = f"{icon} **{sid} {name}**: `{round(close_price, 2)}` ({p_percent:+.2f}%) | 量增: {growth}x"
